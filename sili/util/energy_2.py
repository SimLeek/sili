...
class CPUActivationSparsityManager:
    def __init__(self, max_temp=80.0, goal_hz=10.0, min_sparsity=0.001, max_sparsity=0.5, adj_per_sec = 0.1):
        self.max_temp = max_temp
        self.goal_hz = goal_hz
...
class CPUSynapseNeuronManager:
    def __init__(self, ram_setpoint=0.8, swap_setpoint=0.8, synapse_size=12, neuron_size=12, sips_max = 100_000_000, sops_max = 100_000_000, swap_time_window=5.0, swap_measure_interval=0.1):
        ...
        self.synapse_size = synapse_size
        self.neuron_size = neuron_size
        self.sips_max = sips_max
        self.sops_max = sops_max
        self.max_synapses = 0
        self.max_neurons = 0
        self.prev_time = None
        self.swap_in = []
...
class HDDSynapseNeuronManager:
    ...
    def update(self, metrics):
        ...
        self.max_synapses = int(available_mb * 1e6 / self.synapse_size)
        self.max_neurons = int(available_mb * 1e6 / self.synapse_size)
        return self.max_synapses, self.max_neurons
...
if __name__ == "__main__":
    ...
    # Compute-centric deques
    cpu_temp_data = deque(maxlen=max_points)
    cpu_percent_data = deque(maxlen=max_points)
    cpu_freq_data = deque(maxlen=max_points)
    cpu_loadavg_data = deque(maxlen=max_points)
    cpu_fan_data = deque(maxlen=max_points)
    gpu_temp_data = [deque(maxlen=max_points) for _ in gpu_managers]
    gpu_fan_data = [deque(maxlen=max_points) for _ in gpu_managers]
    gpu_power_data = [deque(maxlen=max_points) for _ in gpu_managers]
    gpu_util_data = [deque(maxlen=max_points) for _ in gpu_managers]
    hdd_io_data = deque(maxlen=max_points)
    battery_data = deque(maxlen=max_points)

    # Memory-centric deques
    ram_usage_data = deque(maxlen=max_points)
    swap_usage_data = deque(maxlen=max_points)
    hdd_usage_data = deque(maxlen=max_points)
    gpu_mem_data = [deque(maxlen=max_points) for _ in gpu_managers]
    ...
    while True:
        ...
        for i, manager in enumerate(gpu_managers):
            gpu_sparsity_data[i].append(manager.update(metrics)*100)
        ...
        cpu_sparsity_data.append(cpu_manager.update(metrics)*100)
        cpu_synapse_data.append(cpu_synapse_manager.update(metrics)[0] / 1e6)
        hdd_sparsity_data.append(hdd_manager.update(metrics)*100)
        hdd_synapse_data.append(hdd_synapse_manager.update(metrics)[0] / 1e6)

        # Update compute-centric metrics
        cpu_temp_data.append(max(t.current for t in metrics.cpu_temp.get('coretemp', [])) if metrics.cpu_temp.get('coretemp') else 0.0)
        cpu_percent_data.append(sum(metrics.cpu_percent) / len(metrics.cpu_percent) if metrics.cpu_percent else 0.0)
        print(metrics.cpu_percent)
        cpu_freq_data.append(max(f.current for f in metrics.cpu_freq if f) / 1000 if metrics.cpu_freq else 0.0)  # MHz to GHz
        cpu_loadavg_data.append(metrics.cpu_loadavg[0] if metrics.cpu_loadavg else 0.0)  # 1-minute load
        cpu_fan_data.append(max(f.current for f in metrics.cpu_fans.get('cpu_fan', [])) if metrics.cpu_fans.get('cpu_fan') else 0.0)
        for i, gpu in enumerate(metrics.gpu_temps):
            gpu_temp_data[i].append(gpu[3])
            gpu_fan_data[i].append(next((f[3] for f in metrics.gpu_fans if f[2] == gpu[2]), 0.0))
            gpu_power_data[i].append(next((p[3][0] for p in metrics.gpu_power if p[2] == gpu[2]), 0.0))
            gpu_util_data[i].append(next((u[3] for u in metrics.gpu_util if u[2] == gpu[2]), 0.0))
        io_rate = 0.0
        for disk in ['sdc3']:
            counters = metrics.hdd_counters.get(disk, None)
            if counters:
                io_rate += (counters.read_bytes + counters.write_bytes) / 1e12  # MB/s
        print(io_rate)
        hdd_io_data.append(io_rate)
        battery_data.append(metrics.battery_level.percent if hasattr(metrics.battery_level, 'percent') else 0.0)

        # Update memory-centric metrics
        ram_usage_data.append(metrics.ram_usage.used / 1e9 if metrics.ram_usage else 0.0)  # Bytes to GB
        swap_usage_data.append(metrics.swap_usage.used / 1e9 if metrics.swap_usage else 0.0)  # Bytes to GB
        hdd_usage_data.append(metrics.hdd_usage.used / 1e9 if metrics.hdd_usage else 0.0)  # Bytes to GB
        for i, gpu in enumerate(metrics.gpu_mem):
            gpu_mem_data[i].append(gpu[3][0])  # Used memory in MB

        # Plot compute-centric metrics (ax1)
        ax1.clear()
        ...
        ax1.plot(times, cpu_temp_data, label='CPU Temp (°C)', color='red', linestyle='--')
        ax1.plot(times, cpu_percent_data, label='CPU %', color='purple', linestyle='--')
        ax1.plot(times, cpu_freq_data, label='CPU Freq (GHz)', color='orange', linestyle='--')
        ax1.plot(times, cpu_loadavg_data, label='CPU Load Avg', color='cyan', linestyle='--')
        ax1.plot(times, cpu_fan_data, label='CPU Fan (RPM)', color='magenta', linestyle='--')
        for i, temp in enumerate(gpu_temp_data):
            ax1.plot(times, temp, label=f'GPU{i} Temp (°C)', color=f'C{i+2}', linestyle='-.')
        for i, fan in enumerate(gpu_fan_data):
            ax1.plot(times, fan, label=f'GPU{i} Fan (%)', color=f'C{i+4}', linestyle='-.')
        for i, power in enumerate(gpu_power_data):
            ax1.plot(times, power, label=f'GPU{i} Power (W)', color=f'C{i+6}', linestyle='-.')
        for i, util in enumerate(gpu_util_data):
            ax1.plot(times, util, label=f'GPU{i} Util (%)', color=f'C{i+8}', linestyle='-.')
        ax1.plot(times, hdd_io_data, label='HDD I/O (MB/s)', color='brown', linestyle='--')
        ax1.plot(times, battery_data, label='Battery (%)', color='gray', linestyle='--')
        ax1.set_title('Sparsity and Compute Metrics over Time')
        ax1.set_xlabel('Time (s)')
        ax1.set_ylabel('Value')
        ax1.legend(loc='upper right', fontsize='small')
        ax1.grid(True)

        # Plot memory-centric metrics (ax2)
        ax2.clear()
        ...
        ax2.plot(times, ram_usage_data, label='RAM Used (GB)', color='red', linestyle='--')
        ax2.plot(times, swap_usage_data, label='Swap Used (GB)', color='purple', linestyle='--')
        ax2.plot(times, hdd_usage_data, label='HDD Used (GB)', color='orange', linestyle='--')
        for i, mem in enumerate(gpu_mem_data):
            ax2.plot(times, mem, label=f'GPU{i} Mem Used (MB)', color=f'C{i+2}', linestyle='-.')
        ax2.set_title('Synapses and Memory Usage')
