...
uint64_t bitwise_add_simd_4bit_64(uint64_t a, uint64_t b) {
    uint64_t result = 0;
    uint64_t carry = 0;
    uint64_t mask = 0x1111111111111111ULL; // Mask for active bits
...
uint64_t bitwise_subtract_simd_4bit_64(uint64_t a, uint64_t b) {
    uint64_t result = 0;
    uint64_t borrow = 0;
    uint64_t mask = 0x1111111111111111ULL; // Mask for active bits
...
uint64_t bitwise_multiply_simd_4bit_64(uint64_t a, uint64_t b) {
    uint64_t result = 0;
    uint64_t mask = 0x1111111111111111ULL; // Mask for least significant bits of all 4-bit numbers
...
uint64_t bitwise_divide_simd_4bit_64(uint64_t a, uint64_t b) {
    uint64_t result = 0;
    uint64_t remainder = 0;
    uint64_t mask = 0x8888888888888888ULL; // Mask for most significant bits of all 4-bit numbers

    for (int i = 0; i < 4; i++) {
        // Shift remainder and bring down next bit from a
        remainder = (remainder << 1) | ((a & mask) ? 0x1111111111111111ULL : 0);
        
        // Check if we can subtract b from remainder
        uint64_t can_subtract = 0;
        uint64_t temp_remainder = remainder;
        uint64_t shifted_b = b & 0x0F0F0F0F0F0F0F0FULL; // Consider only lower 4 bits of each packed number in b
        ...
        can_subtract = (temp_remainder & 0x8888888888888888ULL) ? 0 : 0x1111111111111111ULL;
        
        // Update result and remainder
        result = (result << 1) | can_subtract;
        remainder = can_subtract ? (temp_remainder & 0x0F0F0F0F0F0F0F0FULL) : remainder;
