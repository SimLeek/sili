std::vector<cs2_struct> outer_product(
    int batches,
    int a_size,
    int b_size,
    const cs2_struct& a,
    ...
    for (int batch = 0; batch < num_batches; ++batch) {
        ...
        result_batches.emplace_back(&result_indices, &result_values, nullptr, a_size, b_size, nnz);
    }

    return result_batches;
}

void outer_product_backwards_b(
    int batches,
    int a_size,
    int b_size,
    const cs2_struct& a,
    const cs2_struct& b_grad,
    std::vector<cs2_struct> o_grad_b
) {
    // output is a list of length batches of CSRs of size a_size by b_size
    int num_cpus = std::thread::hardware_concurrency();
    std::vector<CSR> result_batches;
    ...
    for (int batch = 0; batch < num_batches; ++batch) {
        ...
        #pragma omp parallel num_threads(num_cpus)
        {
            ...
            for (int i = start; i < end; ++i) {
                ...
                int row_idx = a_batches.indices[a_index];
                ...
                for (int j = b_grad_start; j < b_grad_end; ++j) {
                    ...
                    // then sent it here
                    int o_start = o_grad[batch].ptrs[row_idx];
                    float o_val = o_grad[batch].values[j];
...
void outer_product_backwards_a(
    int batches,
    int a_size,
    int b_size,
    const cs2_struct& a_grad,
    const cs2_struct& b,
    std::vector<cs2_struct> o_grad_a
) {
    // output is a list of length batches of CSRs of size a_size by b_size
    int num_cpus = std::thread::hardware_concurrency();
    std::vector<CSR> result_batches;
    ...
    for (int batch = 0; batch < num_batches; ++batch) {
        ...
        int a_grad_end =a_grada.ptrs[batch + 1];
        int num_rows = a_grad_end - a_grad_start;

        int b_start = b.ptrs[batch];
        int b_end = b.ptrs[batch + 1];
        int num_cols = b_end - b_grad_start;
...
std::vector<cs2_struct> build_outer_product_mask(
    int batches,
    int a_size,
    int b_size,
    const cs2_struct& a,
    ...
    for (int batch = 0; batch < num_batches; ++batch) {
        ...
        // Convert the result to CSR format for the current batch
        result_batches.emplace_back(&result_indices, nullptr, nullptr, a_size, b_size, nnz);
    }
