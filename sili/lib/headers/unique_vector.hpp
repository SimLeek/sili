...
# ifndef _POD_VECTOR_HPP
# define _POD_VECTOR_HPP
...
//non-copyable vector. Because there's no point in trying to make fast code when arrays are copied a million times behind the scenes
template <class Type>
class unique_vector {
private:
     size_t length_=0;
...
public:
     ...
     explicit unique_vector(unique_vector&& src): length_(std::move(src.length_)), capacity_(std::move(src.capacity_)), data_(std::move(src.data_))
     {  
     }
     unique_vector(void)
     : length_(0), capacity_(0), data_(nullptr)
     {    //ASSERT( std::is_standard_layout<size_t>() && std::is_trivial<size_t>(), "not plain old data");
     }
     unique_vector(
          size_t n
     )   : length_(0), capacity_(0), data_(nullptr)
     ...
     /*template<typename = std::enable_if_t<!std::is_same_v<Type, std::initializer_list<Type>>>>
    unique_vector(const std::initializer_list<Type>&& init_list)
    : length_(0), capacity_(0), data_(nullptr)
    { 
        ...
        if constexpr( !(std::is_standard_layout<Type>() && std::is_trivial<Type>()))
          {
               ...
               size_t i=0;
                    for(auto&& elem : init_list){
                         //new(data_ + i) Type();
                         new(&data_[i]) Type(std::move(elem));
                         i++;
                    }
               }else{
               ...
               std::move(init_list.begin(), init_list.end(), data_);
          }
    }*/

     template<typename wrong_type, typename = std::enable_if<!is_specialization<wrong_type, unique_vector>::value>>
    unique_vector(const std::initializer_list<wrong_type>& init_list)
    : length_(0), capacity_(0), data_(nullptr)
    { 
        extend(init_list.size());
        if constexpr( !(std::is_standard_layout<Type>() && std::is_trivial<Type>()))
    ...
    /*unique_vector&& operator=(const std::initializer_list<Type>&& init_list)
    { 
        extend(init_list.size());
        if constexpr( !(std::is_standard_layout<Type>() && std::is_trivial<Type>()))
          {
               ...
               size_t i=0;
                    ...
                    throw std::invalid_argument("Attempted to initialize a non copy constructable type (Note: don't use double initializer lists)");
               }
          }else{
               std::move(init_list.begin(), init_list.end(), data_);
          }
    ...
    unique_vector&& operator=(const std::initializer_list<wrong_type>&& init_list)
    { 
        extend(init_list.size());
        if constexpr( !(std::is_standard_layout<Type>() && std::is_trivial<Type>()))
          {
               ...
               size_t i=0;
                    ...
                    throw std::invalid_argument("Attempted to initializer a non copy constructable type (Note: don't use double initializer lists)");
               }
          }else{
               std::copy(init_list.begin(), init_list.end(), data_);
          }
    ...
    explicit unique_vector(Args&&... args)
        : length_(sizeof...(args)), capacity_(sizeof...(args)), data_(nullptr)
    {
        data_ = static_cast<Type*>(malloc(capacity_ * sizeof(Type)));
        ASSERT(data_ != nullptr, "Memory allocation failed");

        size_t i = 0;
        // Use perfect forwarding to construct elements in place
        (new(&data_[i++]) Type(std::forward<Args>(args)), ...);
    }*/

    /*template<typename... Args>
    explicit unique_vector(const std::initializer_list<Args>&&... args)
        : length_(sizeof...(args)), capacity_(sizeof...(args)), data_(nullptr)
    {
        data_ = static_cast<Type*>(malloc(capacity_ * sizeof(Type)));
        ASSERT(data_ != nullptr, "Memory allocation failed");
        ...
        size_t i = 0;
        // Use perfect forwarding to construct elements in place
        (new(&data_[i++]) Type(std::forward<const std::initializer_list<Args>&&>(args)), ...);
    }*/
     ~unique_vector(void)
     {    if( length_ > 0 )
     ...
     {    size_t old_length   = length_;
          ...
          void* v_ptr;
          if constexpr( !(std::is_standard_layout<Type>() && std::is_trivial<Type>())){
               v_ptr = calloc(length_<<1, sizeof(Type));  // avoid uninitialized access for pod vectors of pod vectors for example
          }else{
     ...
     {    ASSERT( i < length_, "Attempted to access out of bounds element "+std::to_string(i)+". Max: "+std::to_string(length_));
          return data_[i];
     }
    ...
    const Type& back() const { 
     ...
     bool operator==(const unique_vector<OTHER_TYPE>& b) const { 
     return std::distance(begin(), end()) == std::distance(b.begin(), b.end()) && std::equal(begin(), end(), b.begin());
    }

     /*void erase(void)  // this is wrong you fucking retard. I might as well remove the copyright from this shit code because I'm using 0% of it now.
     {    length_ = 0;
          return;
     }*/
     ...
     {    size_t i;
          ...
          ASSERT( length_   == x.length_ , "lengths of vectors should be the same when assigning");
          for(i = 0; i < length_; i++)
          {    data_[i] = x.data_[i]; }
     }*/
